...
def main():
    parser = argparse.ArgumentParser(description="Prototype02 완전 통합 텍스트 정렬 시스템")
    ...
    parser.add_argument(
        ...
        help="토크나이저 체인 선택 (기본값: prototype02)"
    )
    ...
    target_tokenizer_type = args.target_tokenizer or args.tokenizer
    ...
    # 실행
    logger.info("=== Prototype02 체인 토크나이저 파이프라인 시작 ===")
    logger.info(f"입력: {args.input_path}")
    logger.info(f"출력: {args.output_path}")
    logger.info(f"토크나이저 체인: {source_tokenizer_type} (원문), {target_tokenizer_type} (번역문)")
