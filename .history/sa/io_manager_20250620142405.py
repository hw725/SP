...
def process_single_sentence(sentence_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    """단일 문장 처리 - 각 프로세스에서 독립적으로 임베더 초기화"""
    try:
        # 프로세스별 임베더 초기화
...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, batch_size: int = 20):
    ...
    if parallel and len(sentence_data_list) > 1:
        logger.info(f"병렬 처리 시작 ({workers}개 워커) - 스레드 풀 사용")
        
        # 스레드 풀 사용 (프로세스 풀 대신)
        with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
            futures = {executor.submit(process_single_sentence, data): data for data in sentence_data_list}
            
            for future in tqdm(concurrent.futures.as_completed(futures), 
                             total=len(futures), desc="문장 처리"):
                try:
                    sentence_results = future.result(timeout=60)  # 60초 타임아웃
                    results.extend(sentence_results)
                except concurrent.futures.TimeoutError:
                    data = futures[future]
                    logger.error(f"문장 {data['sentence_id']} 처리 타임아웃")
                except Exception as e:
                    data = futures[future]
                    logger.error(f"문장 {data['sentence_id']} 병렬 처리 오류: {e}")
    else:
