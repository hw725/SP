"""Prototype02 통합 상태 철저 점검 - 수정된 버전"""
import sys
import os
sys.path.append('src')
...
def check_prototype02_integration():
    ...
    # 7. TextAligner 개별 테스트 추가
    try:
        aligner = TextAligner()
        src_units = ["中國人民", "解放軍은", "强力한", "軍隊입니다"]
        tgt_units = ["Chinese People's", "Liberation Army", "is", "powerful"]
        ...
        aligned = aligner.align(src_units, tgt_units, dummy_embed)
        if aligned and len(aligned) == len(src_units):
            print("✓ TextAligner 기능 정상")
        else:
            issues.append(f"✗ TextAligner 결과 부적절: {aligned}")
    except Exception as e:
        issues.append(f"✗ TextAligner 테스트 실패: {e}")
