...
def split_src_meaning_units(text: str) -> List[str]:
    ...
    jieba_boundaries = set()
    pos = 0
    for token in jieba_tokens:
        pos += len(token)
        jieba_boundaries.add(pos)
    ...
    while i < len(tokens):
        ...
        if hanja_re.search(tok):
            unit = tok
            j = i + 1
            current_pos = text_pos + len(tok)
            
            # jieba 경계가 아니면 병합 계속
            while (j < len(tokens) and 
                   hangul_re.match(tokens[j]) and 
                   current_pos not in jieba_boundaries):
                unit += tokens[j]
                current_pos += len(tokens[j]) + 1
                j += 1
...
def calculate_mecab_completeness(span: str) -> float:
    ...
    try:
        ...
        # 완전한 어절 확인
        has_noun = any(pos.startswith('N') for _, pos in morphs)
        has_verb = any(pos.startswith('V') for _, pos in morphs)
        has_ending = any(pos in ['EF', 'EC'] for _, pos in morphs)
        has_josa = any(pos.startswith('J') for _, pos in morphs)
        
        if has_noun: score += 0.2
        if has_verb: score += 0.2
        if has_ending: score += 0.3
