...
def main():
    ...
    parser.add_argument("--embedder", default=None, help="임베더 타입 (기본값: Config 클래스에서 설정)")
    ...
    try:
        config = Config(
            ...
            embedder_type=args.embedder or Config().embedder_type, # None이면 기본값 사용
        ...
        # 실제 로드된 임베더 클래스 확인
        from src.components import get_embedder
        test_embedder = get_embedder(config.embedder_type, **config.embedder_config)
        logging.info(f"임베더: {config.embedder_type} (클래스: {test_embedder.__class__.__name__}, 캐시: {config.embedder_config.get('cache_dir', '미설정')})")
