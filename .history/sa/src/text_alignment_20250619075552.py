...
def fast_calculate_score(src_embed, tgt_embed, src_unit, tgt_span, i, j, k):
    # 코사인 유사도 (벡터화)
    semantic_sim = np.dot(src_embed, tgt_embed) / (
        np.linalg.norm(src_embed) * np.linalg.norm(tgt_embed) + 1e-8
    )
    
    # 구조 분석 캐싱 (이미 계산된 것 재사용)
    if hasattr(fast_calculate_score, 'structure_cache'):
        cache_key = (src_unit, tgt_span)
        if cache_key in fast_calculate_score.structure_cache:
            structure_sim = fast_calculate_score.structure_cache[cache_key]
        else:
            structure_sim = 0.1  # 간단한 휴리스틱
            fast_calculate_score.structure_cache[cache_key] = structure_sim
    else:
        fast_calculate_score.structure_cache = {}
        structure_sim = 0.1
    
    return 0.6 * semantic_sim + 0.25 * structure_sim
