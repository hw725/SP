...
def split_tgt_by_src_units_with_eojeol_merge(
    src_units: List[str],       # ⬅️ 기존 masked_src 자리에 src_units
    tgt_text: str,              # ⬅️ 기존 tgt_text
    embed_func: Callable,
    ...
    # 사전 계산: 각 src 단위 임베딩과 구조 분석
    src_embeds  = []
    src_analyses = []
    for unit in src_units:
        emb = embed_func([unit])[0]
        src_embeds.append(emb)
        src_analyses.append(
            analyze_unit_structure(unit, source_analyzer, target_analyzer, "source")
        )
