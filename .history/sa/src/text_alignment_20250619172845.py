...
def split_tgt_by_src_units_with_eojeol_merge(
    ...
    while i > 0:
        ...
        if pk > pj:
            ...
            path_info.append(f"src[{pi}] -> tgt[{pj}:{pk}]")
        i, j = pi, pj
    
    logger.info(f"[DEBUG] 매핑 경로: {path_info}")
    logger.info(f"[DEBUG] 처리되지 않은 번역문 범위: 0~{path_info[0] if path_info else 0}, {end_j}~{n_tgt}")
