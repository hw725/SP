...
class Config:
    ...
    embedder_type: str = "openai"                 # 여기 기본값도 변경
...
def run_processing(config: Config):
    logger.info(f"[DEBUG] Loaded Config.embedder_type = {config.embedder_type!r}")
    logger.info(f"[DEBUG] Loaded Config.embedder_config = {config.embedder_config}")
    …